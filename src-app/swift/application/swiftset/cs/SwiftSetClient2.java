...
public class SwiftSetClient2 {
    ...
    static void runClient1Code(String server, String dcName) {
        Endpoint srv = Networking.resolve(server, SwiftSetServer.PORT1);
        Endpoint dc = Networking.resolve(dcName, DCConstants.SURROGATE_PORT);
        client1Code(srv, dc);
    }

    static void runClient2Code(String server) {
        Endpoint srv = Networking.resolve(server, SwiftSetServer.PORT2);
        client2Code(srv);
    }

    static final int timeout = SwiftSetServer.synchronousOps ? 5000 : 0;
