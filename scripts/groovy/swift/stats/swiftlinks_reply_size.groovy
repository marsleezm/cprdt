...
def processFile = { int L, int H, File f, String placement, String series ->
    ...
    f.eachLine { String line ->
        ...
        if (tok.length >= 3) {
            ...
            int msgSize = Integer.valueOf(tok[4])
            ...
            if (T >= L * 1000 && T <= H * 1000) {
                ...
                series_per_time_range[index] << msgSize
            }
        }
    }
}
...
range = 30
processDir(0, 1000, DIR, search, "*", "lazy")

lazy_series = series_per_time_range

search = '-nonlazy'
series_per_time_range = [:]
processDir(0, 1000, DIR, search, "*", "nonlazy")
...
def gnuplot = [
    ...
    'set xlabel "Time [ s ]"',
    'set ylabel "Sum of CRDT state size in messages [ bytes ]"',
    'set mxtics',
    'set mytics',
...
GnuPlot.doGraph( '/tmp/sosp/SwiftLinks-reply-size-sum', gnuplot, plots, { k, v ->
    String.format('title "%s" with linespoints pointinterval 4 lw 3 ps 2', k)
}, keySorter )

GnuPlot.doGraph( '/tmp/sosp/SwiftLinks-reply-size-avg', gnuplot, plots_avg, { k, v ->
    String.format('title "%s" with linespoints pointinterval 4 lw 3 ps 2', k)
}, keySorter )
