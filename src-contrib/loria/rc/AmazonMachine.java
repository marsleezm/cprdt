...
public class AmazonMachine {
    ...
    public void checkAndCreateSecurityGroup() throws Exception {
        try {
            CreateSecurityGroupRequest securityGroupRequest = new CreateSecurityGroupRequest(SECURITYGROUPNAME,
                    "This group is generated by " + this.getClass().getCanonicalName());
        ...
        /*
         * } catch (AmazonServiceException ase) { // Ignore because this likely
         * means the zone has already been authorized.
    ...
    public void waitAllLauched() {
        DescribeInstanceStatusRequest describeInstanceRequest = new DescribeInstanceStatusRequest()
                .withInstanceIds(this.newInstanceIds);
        ...
        List<InstanceStatus> state;// =
                                   // describeInstanceResult.getInstanceStatuses();
        int launched = 0;
        do {
            try {
        ...
        for (String id : newInstanceIds) {
            ...
            Logger.getLogger(ClientModifierBenchmarkJob.class.getName()).info(
                    "add Machine " + dns + " : " + addr.getHostAddress());
        }
    ...
    public boolean instanceIsLauched() {
        ...
        /*
         * while (state.size() < 1) { try { Thread.sleep(5000); } catch
         * (InterruptedException ex) { } // Do nothing, just wait, have thread
         * sleep if needed describeInstanceResult =
         * ec2.describeInstanceStatus(describeInstanceRequest); state =
         * describeInstanceResult.getInstanceStatuses(); }
         */
        for (int i = 0; i < 100; i++) {
            System.out.println("---");
    ...
    public boolean areAnyOpen() {
        ...
        try {
            ...
            // Look through each request and determine if they are all in the
            // active state.
            for (SpotInstanceRequest describeResponse : describeResponses) {
                System.out.println(" " + describeResponse.getSpotInstanceRequestId() + " is in the "
                        + describeResponse.getState() + " state.");

                // If the state is open, it hasn't changed since we attempted to
                // request it.
